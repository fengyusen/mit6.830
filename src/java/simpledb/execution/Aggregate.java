...
public class Aggregate extends Operator {
    ...
    public Aggregate(OpIterator child, int afield, int gfield, Aggregator.Op aop) {
        ...
        Type groupFieldType = gfield == -1 ? null : this.child.getTupleDesc().getFieldType(this.gfield);
        if (this.child.getTupleDesc().getFieldType(this.afield) == (Type.STRING_TYPE)) {
            this.aggregator = new StringAggregator(this.gfield, groupFieldType, this.afield, this.aop);
        } else {
            this.aggregator = new IntegerAggregator(this.gfield, groupFieldType, this.afield, this.aop);
        }
        this.opIterator = this.aggregator.iterator();
//        List<Type> types = new ArrayList<>();
//        List<String> names = new ArrayList<>();
//        if (groupFieldType != null) {
//            types.add(groupFieldType);
//            names.add(this.child.getTupleDesc().getFieldName(this.groupField));
//        }
//        types.add(this.child.getTupleDesc().getFieldType(this.aggregateField));
//        names.add(this.child.getTupleDesc().getFieldName(this.aggregateField));
//        if (aggregateOp.equals(Aggregator.Op.SUM_COUNT)) {
...
//        this.tupleDesc = new TupleDesc(types.toArray(new Type[types.size()]), names.toArray(new String[names.size()]));
    ...
    public TupleDesc getTupleDesc() {
        // some code goes here
        Type groupFieldType = this.gfield == -1 ? null : this.child.getTupleDesc().getFieldType(this.gfield);
        List<Type> types = new ArrayList<>();
        List<String> names = new ArrayList<>();
        //group field
        if (groupFieldType != null) {
...
//        List<Type> types = new ArrayList<>();
//        List<String> names = new ArrayList<>();
//        Type groupFieldType = groupField == -1 ? null : this.child.getTupleDesc().getFieldType(this.groupField);
//        if (groupFieldType != null) {
//            types.add(groupFieldType);
//            names.add(this.child.getTupleDesc().getFieldName(this.groupField));
//        }
