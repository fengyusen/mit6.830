        ...
        for (int i = 1; i <= joins.size(); i++) {
            ...
            for (Set<LogicalJoinNode> set : subsets){
                ...
                for (LogicalJoinNode node : set) {
                    CostCard currCostCard = computeCostAndCardOfSubplan(stats, filterSelectivities,
                            node, set, bestCostCard.cost, planCache);
                    if (currCostCard != null && currCostCard.cost < bestCostCard.cost) {
                        bestCostCard = currCostCard;
                    }
                }
